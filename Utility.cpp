...
void useDefensiveItem(Character* character, Item& item)
{
    ...
    {
        chDw->boostArmor( item.getBoost() * 1.1 );
    }
    else if( auto* chPa = dynamic_cast<Paladin*>(character) )
    {
        //same with paladins
        chPa->boostArmor( item.getBoost() * 1.3 );
    }
    else if( auto* chDrS = dynamic_cast<DragonSlayer*>(character))
    {
        chDrS->boostArmor( item.getBoost() * 1.5 );
    }
...
void useHelpfulItem(Character* character, Item* item)
{
    if( auto* chDw = dynamic_cast<Dwarf*>(character) )
    {
        chDw->boostHitPoints(item->getBoost() * 2);
    }
    else if( auto* chPa = dynamic_cast<Paladin*>(character) )
    {
        chPa->boostHitPoints(item->getBoost() * 1.5);
    }
    else if( auto* chDrS = dynamic_cast<DragonSlayer*>(character))
    {
        chDrS->boostHitPoints(item->getBoost() * 1.25);
    }
    else if( auto* chDr = dynamic_cast<Dragon*>(character) )
    {
...
void useAttackItem(Character* character, Item* item)
{
    if( auto* chDw = dynamic_cast<Dwarf*>(character) )
    {
        chDw->boostAttackDamage(item->getBoost() * 1.5);
    }
    else if( auto* chPa = dynamic_cast<Paladin*>(character) )
    {
        chPa->boostAttackDamage(item->getBoost() * 1.33);
    }
    else if( auto* chDrS = dynamic_cast<DragonSlayer*>(character))
    {
        chDrS->boostAttackDamage(chDrS->getAttackDamage() * item->getBoost());
